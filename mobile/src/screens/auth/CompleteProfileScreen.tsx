...
const ROLES = [
  { id: 'customer', label: 'مسافر', labelEn: 'Customer', desc: 'گاڑی بک کریں' },
...
export default function CompleteProfileScreen({ navigation }: any) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await completeProfile({ name: name.trim(), role, city });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>پروفائل مکمل کریں</Text>
      <Text style={styles.subtitle}>آپ کا خیر مقدم ہے!</Text>
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1.5, borderColor: COLORS.border, borderRadius: 12, padding: 14, fontSize: 17, color: COLORS.text },
